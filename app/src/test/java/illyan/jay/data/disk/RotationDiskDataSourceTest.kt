...
class RotationDiskDataSourceTest : TestBase() {
	...
	private val rotation = DomainRotation(4, 2, Date.from(Instant.ofEpochMilli(random.nextLong(now.toEpochMilli() - TimeUtils.DAY_IN_MILLIS, now.toEpochMilli()))), 4, 4f, 4f, 4f)
	private val roomRotation = RoomRotation(4, 2, rotation.time.toInstant().toEpochMilli(), 4, 4f, 4f, 4f)
	private val rotations = listOf(
		DomainRotation(1, 1, Date.from(Instant.ofEpochMilli(random.nextLong(now.toEpochMilli() - TimeUtils.DAY_IN_MILLIS, now.toEpochMilli()))), 1, 1f, 1f, 1f),
		DomainRotation(2, 1, Date.from(Instant.ofEpochMilli(random.nextLong(now.toEpochMilli() - TimeUtils.DAY_IN_MILLIS, now.toEpochMilli()))), 2, 2f, 2f, 2f),
		DomainRotation(3, 1, Date.from(Instant.ofEpochMilli(random.nextLong(now.toEpochMilli() - TimeUtils.DAY_IN_MILLIS, now.toEpochMilli()))), 3, 3f, 3f, 3f),
		rotation
	)
	...
	fun `Get all rotations for a particular session`(sessionId: Long) = runTest {
		every { mockedDao.getRotations(sessionId) } returns flowOf(roomRotations.filter { it.sessionId == sessionId })
	...
	fun `Save multiple rotation data`() {
		...
		verify(exactly = 1) { mockedDao.upsertRotations(rotations.map(DomainRotation::toRoomModel)) }
	}
}