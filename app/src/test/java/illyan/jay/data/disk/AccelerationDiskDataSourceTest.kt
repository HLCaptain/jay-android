...
class AccelerationDiskDataSourceTest : TestBase() {
	...
	private val acceleration = DomainAcceleration(4, 2, Date.from(Instant.ofEpochMilli(random.nextLong(now.toEpochMilli() - TimeUtils.DAY_IN_MILLIS, now.toEpochMilli()))), 4, 4f, 4f, 4f)
	private val roomAcceleration = RoomAcceleration(4, 2, acceleration.time.toInstant().toEpochMilli(), 4, 4f, 4f, 4f)
	private val accelerations = listOf(
		DomainAcceleration(1, 1, Date.from(Instant.ofEpochMilli(random.nextLong(now.toEpochMilli() - TimeUtils.DAY_IN_MILLIS, now.toEpochMilli()))), 1, 1f, 1f, 1f),
		DomainAcceleration(2, 1, Date.from(Instant.ofEpochMilli(random.nextLong(now.toEpochMilli() - TimeUtils.DAY_IN_MILLIS, now.toEpochMilli()))), 2, 2f, 2f, 2f),
		DomainAcceleration(3, 1, Date.from(Instant.ofEpochMilli(random.nextLong(now.toEpochMilli() - TimeUtils.DAY_IN_MILLIS, now.toEpochMilli()))), 3, 3f, 3f, 3f),
		acceleration
	)
	...
	fun `Get all accelerations for a particular session`(sessionId: Long) = runTest {
		every { mockedDao.getAccelerations(sessionId) } returns flowOf(roomAccelerations.filter { it.sessionId == sessionId })
	...
	fun `Save multiple acceleration data`() {
		...
		verify(exactly = 1) { mockedDao.upsertAccelerations(accelerations.map(DomainAcceleration::toRoomModel)) }
	}
}